...
CC = gcc
CFLAGS = -O3 -g -pipe -fomit-frame-pointer -Wall -Wextra
CFLAGS_MP = 
#CFLAGS_MP = -fopenmp
...
%:: %.c libsuperiso.a
	$(CC) -I/src/gsl/ -c -O0 -g $(CFLAGS) $(CFLAGS_MP) $*.c;
	$(CC) -o $*.x $(CFLAGS) $(CFLAGS_MP) $(CINCLUDE) $*.o -L/src/gsl/ -lsuperiso -lm -lgsl -lgslcblas;
	@rm -f $*.o;
	@touch $*.x

%.c:: %.c libsuperiso.a
	$(CC) -I/src/gsl/ -c $(CFLAGS_MP) $(CFLAGS) $@;
	$(CC) -o $*.x $(CFLAGS) $(CFLAGS_MP) $(CINCLUDE) $*.o -L/src/gsl/ -lsuperiso -lm -lgsl -lgslcblas;
	@rm -f $*.o;
	@touch $*.x
...
save:
	...
	cp -p cw_nmfv_slha.c superiso_$(VERSION)/;\			
