...
/* reads the SLHA file "name" and puts all the values into the "param" structure */
{
  ...
  errno = 0; char msg[100];
	lecture = fopen(name,"r");
  if(lecture == NULL){
    sprintf(msg,"fopen failed : errno = %d\n",errno);
    WARNING(msg);
    return 0;
  }
	...
	while(EOF != fscanf(lecture,"%s",dummy))
	{
        ...
        else if(!strcasecmp(dummy,"NMFVPAR"))
        {
            while(EOF != fscanf(lecture, "%s", dummy) && strcasecmp(dummy,"Block"))
            {
                if(!strncasecmp("#", dummy,1)) while((EOF!=fscanf(lecture,"%c",dummy))&&(strncasecmp("\n",dummy,1)));
                ...
                {
                    default: while ((EOF!=fscanf(lecture,"%c",dummy))&&(strncasecmp("\n",dummy,1))) break;
                    case 1: fscanf(lecture,"%lf", &param->delta_u_23_LR); break;
                    case 2: fscanf(lecture,"%lf", &param->delta_u_23_LL); break;
                    case 3: fscanf(lecture,"%lf", &param->delta_u_33_LR); break;
                    case 4: fscanf(lecture,"%lf", &param->delta_d_23_LL); break;
                    case 5:  fscanf(lecture,"%lf", &param->delta_d_23_RR); break;
                    case 6: fscanf(lecture,"%lf", &param->delta_d_23_RL); break;
                    case 7: fscanf(lecture,"%lf", &param->delta_d_23_LR); break;
                    case 8: fscanf(lecture,"%lf", &param->delta_d_33_RL); break;
                    case 9: fscanf(lecture,"%lf", &param->delta_d_33_LR); break;
                    case 10: fscanf(lecture,"%lf", &param->M_sq); break;
//                    case 11: fscanf(lecture,"%lf", &param->mass_gluino); break;
//                    case 12: fscanf(lecture,"%lf", &param->mass_cha1); break; 
//                    case 13: fscanf(lecture,"%lf", &param->mass_cha2); break;
                }
...
/// Written to generate an slha2 file from the parameters in param, in the context of NMFV. 
///     Any other usage is not currently supported.   
    char tmp[50];
    if(param->FV == 0){
        sprintf(tmp, "undefined model, program will exit");
        ERROR(tmp);
    } // else, param should be non empty ? 
  
    FILE * output = fopen(name_output, "w");
    fprintf(output,"BLOCK MODSEL\t #Model Selection\n");
    fprintf(output, "\t 1\t0\t #MSSM \n");
    fprintf(output, "\t6\t1\t #FV on \n");
