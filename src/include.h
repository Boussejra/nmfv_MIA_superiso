...
/* structure containing all the scanned parameters from the SLHA file */
{
    ...
    // NMFV-specific parameters : 
    double delta_u_23_LR, delta_u_23_LL, delta_u_33_LR; // flavor violation for up-type, 2,3-gen squarks
    double delta_d_23_LL, delta_d_23_RR, delta_d_23_RL, delta_d_23_LR, delta_d_33_LR ,delta_d_33_RL ; // ibid for down types
    double M_sq ;
	/* SLHA2 */
	int NMSSM,RV,CPV,FV;
...
int slha2_generator(char name_output[], struct parameters * param);
/* alphas.c */
...
// Pretty print for error handling : 
...
int sgn( double var );
// Struct initializer : sets required arg to -1, and sets default values for opt args.
void init_Inputvar( Input_var *var );

// Calculate Physical masses.
void input_masses( Input_var *var, struct parameters * param);
// Calculate U,V mixing matrices from input.
void input_charg_UVmixing(Input_var * var, struct parameters * param);
// Sets yukawa from input in MSSM framework
void set_yukawa( Input_var * var, struct parameters *param );
//void set_CKM(struct parameters * param);
/* Useful functions to compute the contributions (NMFV) */

// Special functions
bool is_int(double x);
long double factorial (double x);
int Krodelta(int i,int j); /// Kronecker delta function
double Beta_E(double i, double j); // Uses : tgamma function of GNU C std math lib
...
double hypgeo2F1(double a, double b, double c, double z);// Method : Taylor Series.
double hypgeo2F1_Bateman_2_10_eq19(int a, int b, int c, double z); // Bateman, analytic continuation with a,b,c integers. 
//double hypgeo2F1_transform(double a, double b, double c, double z);

// Computing of loop integrals based on analytical expressions obtained (Amine)
